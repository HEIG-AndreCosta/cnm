...
int16x8_t sobel_x = { 1,  0, -1,
                      2,     -2,
                      1,  0, -1};

int16x8_t sobel_y = { 1,  2,  1,
                      0,      0,
                     -1, -2, -1};
...
{       
    ...
    std::string base_name = image_file.substr(0, image_file.find_last_of('.'));
    ...
    if(src.type() != CV_8UC1)
    {
        std::cout << "Error reading image as grayscale " << argv[1] << std::endl;        
        return EXIT_FAILURE;
    }
    ...
    for(size_t i = 0; i < image_rows; ++i)
    {
        for(size_t j = 0; j < image_cols; ++j)
        {
            ...
            double value = std::sqrt(std::pow(Gx[i][j], 2) + std::pow(Gy[i][j], 2));
    ...
    imwrite(base_name+"_x.png", Mat(image_rows, image_cols, CV_16SC1, Gx));    
    imwrite(base_name+"_y.png", Mat(image_rows, image_cols, CV_16SC1, Gy));   
